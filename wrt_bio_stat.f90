subroutine wrt_bio_stat
  ...
  ALLOCATE(DumpBio(grd%im,grd%jm,grd%km))
  ...
  do m=1,bio%ncmp
    do l=1,bio%nphy
      ...
      ierr = nf90mpi_def_var(ncid, MyVarName, nf90_float, (/xid,yid,depid/), idP )
      ...
      if(iVar .le. 4) then
        ...
        do k=1,grd%km
          do j=1,grd%jm
            do i=1,grd%im
              DumpBio(i,j,k) = REAL(bio%pquot(i,j,k,l), 4)
            enddo
          enddo
        enddo
        ...
        do k=1,grd%km
          do j=1,grd%jm
            do i=1,grd%im
              DumpBio(i,j,k) = REAL(bio%cquot(i,j,k,l,m), 4)
            enddo
          enddo
        enddo

        ierr = nf90mpi_put_var_all(ncid,idP,bio%cquot(:,:,:,1,2),MyStart,MyCount)
        if (ierr .ne. NF90_NOERR ) call handle_err('nf90mpi_put_var_all '//bio%DA_VarList(iVar), ierr)
      endif
