...
!  ALLOCATE(Nut(grd%im,grd%jm,grd%km)); Nut(:,:,:) = dble(0.)
  ...
  if(MyId .eq. 0) then
    write(drv%dia,*) 'writing oxy structure'     
  endif
  ...
  if(bio%o2o .eq. 0) then
    write(*,*) "ERROR: Oxygen to be assimilated NOT set in namelist"
    write(drv%dia,*) "ERROR: Oxygen to be assimilated NOT set in namelist"
    call MPI_Barrier(Var3DCommunicator, ierr)
    call MPI_Abort(Var3DCommunicator,-1,ierr)
  endif
  
  if((bio%o2o .eq. 1) .and. ((NNutVar-NNVar).lt.1)) then
    write(*,*) "ERROR: Oxygen to be assimilated but NOT set in DA_params.f90"
    write(drv%dia,*) "ERROR: Oxygen to be assimilated but NOT set in DA_params.f90"
    call MPI_Barrier(Var3DCommunicator, ierr)
    call MPI_Abort(Var3DCommunicator,-1,ierr)
  endif
    ...
    do j=1,grd%jm
      do i=1,grd%im
        if(bio%InitialNut(i,j,k,1) .lt. 1.e20) then
          ...
          ValuesToTest(i,j,k) = bio%InitialNut(i,j,k,NNVar+1) + grd%o2o(i,j,k)
            endif
          enddo
        enddo
      enddo
        ...
        if(bio%InitialNut(i,j,k,NNVar+1) .lt. 1.e20) then
          ...
          if(grd%msk(i,j,k).eq.1) then
            ...
            if(ValuesToTest(i,j,k) .lt. 0) then
              ...
              DumpBio(i,j,k,1) = ValuesToTest(i,j,k)
            ...
            DumpBio(i,j,k,1) = bio%InitialNut(i,j,k,NNVar+1)
          endif
